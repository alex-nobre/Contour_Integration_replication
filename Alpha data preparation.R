...
sqr1.alpha.files <- list.files('./Data/Data_BVA', pattern = "1_Alpha ROI Sqr_FFTBandExport")
sqr2.alpha.files <- list.files('./Data/Data_BVA', pattern = "2_Alpha ROI Sqr_FFTBandExport")
rand1.alpha.files <- list.files('./Data/Data_BVA', pattern = "1_Alpha ROI Rand_FFTBandExport")
rand2.alpha.files <- list.files('./Data/Data_BVA', pattern = "2_Alpha ROI Rand_FFTBandExport")
...
alpha.line <- ggplot(rep_data_long2, aes(x = group, y = alpha, 
  ...
  facet_grid(.~session) +
  labs(title = "Alpha mean power with exclusion", x = "new group", y = "Alpha mean", 
       colour = "configuration")
alpha.line

rep_data_long2 <- rep_data_long2[-which(rep_data_long2$Subject == 33),]

plot(1:length(rep_data_long2$Subject), rep_data_long2$alpha)
