...
rep_data_long2 <- reshape(rep_data_long, varying = 5:30, sep = "_", direction = "long")

rep_data5 <- rep_data2 %>%
              unite(one,contains("1")) %>%  
              unite(two, contains("2")) %>% 
              unite(three, contains("3")) %>% 
              gather(replicate,values,one:three) %>%
              separate(values,c("C1.sqr","STRS"),sep = "_") 

# Convert from wide to long
# Extracts session number to column
rep_data_long <- reshape(rep_data, varying = c("C1.sqr_1", "C1.sqr_2", "C1.rand_1",
