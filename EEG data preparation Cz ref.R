...
# Merge data frames
rep_data_cz <- cbind(sqr1_dat, sqr2_dat, rand1_dat, rand2_dat)

# Add group variable
rep_data_cz$group <- c("unaware", "aware", "unaware", "unaware", "unaware", 
...
rep_data_cz$group.original <- c("unaware", "aware", "unaware", "unaware", "aware", 
                                "aware", "unaware", "aware", "aware", "aware", "aware", 
...
rep_data_cz <- cbind(Subject, rep_data_cz)

# Remove subjects with too many artifacts
rep_data_cz2 <- rep_data_cz[-c(25, 26),]

# Convert from wide to long
# Rename columns to separate session number using sep = "_"
names(rep_data_cz2)[names(rep_data_cz2) == "occ_sqr_1"] <- "occ.sqr_1"
names(rep_data_cz2)[names(rep_data_cz2) == "occ_sqr_2"] <- "occ.sqr_2"
names(rep_data_cz2)[names(rep_data_cz2) == "occ_rand_1"] <- "occ.rand_1"
names(rep_data_cz2)[names(rep_data_cz2) == "occ_rand_2"] <- "occ.rand_2"
names(rep_data_cz2)[names(rep_data_cz2) == "left_sqr_1"] <- "left.sqr_1"
names(rep_data_cz2)[names(rep_data_cz2) == "left_sqr_2"] <- "left.sqr_2"
names(rep_data_cz2)[names(rep_data_cz2) == "left_rand_1"] <- "left.rand_1"
names(rep_data_cz2)[names(rep_data_cz2) == "left_rand_2"] <- "left.rand_2"
names(rep_data_cz2)[names(rep_data_cz2) == "right_sqr_1"] <- "right.sqr_1"
names(rep_data_cz2)[names(rep_data_cz2) == "right_sqr_2"] <- "right.sqr_2"
names(rep_data_cz2)[names(rep_data_cz2) == "right_rand_1"] <- "right.rand_1"
names(rep_data_cz2)[names(rep_data_cz2) == "right_rand_2"] <- "right.rand_2"
# Extracts session number to column
rep_data_cz_long <- reshape(rep_data_cz2, varying = c("occ.sqr_1", "occ.sqr_2",
...
# Rename name of session column
names(rep_data_cz_long)[names(rep_data_cz_long) == "time"] <- "session"

# Rename columns to separate configuration number using sep = "_"
names(rep_data_cz_long)[names(rep_data_cz_long) == "occ.sqr"] <- "occ_1"
names(rep_data_cz_long)[names(rep_data_cz_long) == "occ.rand"] <- "occ_2"
names(rep_data_cz_long)[names(rep_data_cz_long) == "left.sqr"] <- "left_1"
names(rep_data_cz_long)[names(rep_data_cz_long) == "left.rand"] <- "left_2"
names(rep_data_cz_long)[names(rep_data_cz_long) == "right.sqr"] <- "right_1"
names(rep_data_cz_long)[names(rep_data_cz_long) == "right.rand"] <- "right_2"
# Extracts configuration number to column
rep_data_cz_long2 <- reshape(rep_data_cz_long, varying = c("occ_1", "occ_2",
                                                           "left_1", "left_2",
                                                           "right_1", "right_2"),  
                             direction = "long", idvar = " Subject", sep = "_")
# Rename configuration column name
names(rep_data_cz_long2)[names(rep_data_cz_long2) == "time"] <- "configuration"
# Rename configuration levels
rep_data_cz_long2$configuration[rep_data_cz_long2$configuration == 1] <- "sqr"
rep_data_cz_long2$configuration[rep_data_cz_long2$configuration == 2] <- "rand"
rep_data_cz_long2[,9]<- NULL
